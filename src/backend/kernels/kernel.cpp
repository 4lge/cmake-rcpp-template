...
#define REAL_ONE 1.0f
    ...
    +R(
       ...
       real_t exp_rand(__private real_t rate,
                       __private int* idx, __private uint* mt) {
         return 1.0/rate * (-log(mt_rand_01(&mt[0], &idx, mt)));
       }
       ...
       real_t t_rand(__private int nu, 
                     __private int* idx, __private uint* mt) {
         real_t V = nu * (-log(mt_rand_01(&mt[0], &idx[0], mt))); // Chi-squared with ν degrees of freedom
         return sqrt(-2 * log(mt_rand_01(&mt[0], &idx[0], mt))) *
           cos(2 * 3.14159265359f * mt_rand_01(&mt[0], &idx[0], mt)) * sqrt(nu / V);
       }
       ...
       // gamma distribution 
       //       real_t gamma_rand(__private real_t theta -> 1/rate , __private real_t k -> shape,

       real_t gamma_rand(__private real_t shape, __private real_t scale,
                         __private int* idx, __private uint* mt) {
       ...
       real_t weibull_rand(__private real_t lambda, __private real_t k,
                           __private int* idx, __private uint* mt) {
         //return lambda * pow(-log(mt_rand_01(&mt[0], &idx[0], mt))*1.0f, 1.0f / (float)k);
         return lambda *  1.0 / k; 
       }
    ...
    // gamma kernel
    +R(
       kernel void gamma_rng(global real_t* output, global int* seed, global real_t* shape, global real_t* rate) {
         ...
         // use scale=1/rate:
         real_t r_gamma = gamma_rand(*shape, 1.0/(*rate), &idx, mt);
    ...
    +R(
       kernel void exp_rng(global real_t* output, global int* seed, global real_t* rate) {
         ...
         real_t r_exp = exp_rand(*rate, &idx, mt);
    ...
    +R(
       kernel void weibull_rng(global real_t* output, global int* seed, global real_t* lambda, global real_t* k) {
         ...
         real_t r_weibull = 42.0;// weibull_rand(*lambda, *k, &idx, mt);
